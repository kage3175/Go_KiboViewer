...
def atstart() :
    ...
    l1=Label(root, text='깔 돌의 개수는?(호선은 0, 홀수 개의 돌을 까는 경우 돌이 n-1개만 올라가있으며, 첫 수는 흑이 두게 되므로 나머지 돌 까는 위치에 놓고 시작)')
    l1.place(x=10, y=10)
    ee1=Entry(root)
    ee1.place(x=10, y=60)
    ent=Button(root, text='확인', command=okay)
