...
def saveas() : #기보 저장을 할 때 실행되는 함수. tkinter로 기보 이름과 대국자, 승자 등의 정보를 입력받는다.
    ...
    enter=Button(window, text='    저장    ', command=save) # 저장 버튼
    enter.place(x=50, y=210)
    cancel=Button(window, text='    취소    ', command=Cancel) # 취소 버튼
    cancel.place(x=150, y=210)
    window.mainloop()
...
def kibosave() :
    ...
    while 1 : # 기보 만들기 종료시까지 계속 실행하도록
        ...
        if www==2 : #무르기를 실행한 경우
            ...
            now2=fornow[n-1][1]
            sequence[n]=[-1,-1] #기보 저장을 위한 sequence에서 n번째 수순은 초기화해주어야 한다. 그래야 기보 저장 시 오류가 발생하지 않는다.
            cnt-=1
