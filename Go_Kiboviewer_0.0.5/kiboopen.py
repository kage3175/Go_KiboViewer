...
def maketklistbox():
    ...
    frm=Frame(win, width=400, height=250)
    frm.pack()

    listbox=Listbox(win, cursor='arrow', height=10, width=52)
    for i in range(lengthoffile) :
        listbox.insert(i+1,filename[i])
    listbox.place(x=10, y=10)
    ent=Button(win, text='열기', command=oopen)
    ent.place(x=175, y=200)
    win.mainloop()
...
def kiboopen() :
    ...
    pygame.display.set_caption('기보 재생')
    ...
    openkibo=open('./doc/'+ffiillee+'.txt', 'r') # 입력받은 기보의 이름을 토대로 기보를 불러온다.
    ...
    while 1 :
        ...
        for event in pygame.event.get() :
            ...
            if event.type==MOUSEBUTTONDOWN and event.button==LEFT :
                ...
                if position[0]>=51 and position[0]<=601 and position[1]>=51 and position[1]<=601 :
